...
| `walkie-client-java` | Desktop client: a window by default, `--no-gui` for the terminal. `javax.sound.sampled` capture/playback, on virtual threads.     |
...
| `--channel <name>`           | **required**             | Channel to join — **1–64 letters, digits or spaces in any language** (Hebrew, Han, Greek), plus `_` and `-`; no dots. Quote it if it has spaces: `--channel "my room"`. Required for every mode except global, which always uses the reserved `global` room.                                                                                                                                                                                                                                                          |
...
| `--gui`                      | **on** (window)          | Open a **window** — the default; pass `--no-gui` for the terminal prompt instead. The window adds **hold-to-talk** — press and hold the Talk button, or the space bar — which a console cannot offer, since a keystroke has no press/release edges, and it has a Connect form, so it opens without `--display`/`--channel`/`--key`. Where there is no display (`ssh` without X11, a container, CI) the terminal prompt runs instead and says so. Same client underneath: identical audio, encryption and floor rules. |
| `--no-gui`                   | off                      | Run at the **terminal prompt** instead of opening a window; shorthand for `--gui=false`. Wins if `--gui` is given as well.                                                                                                                                                                                                                                                                                                                                                                                            |
