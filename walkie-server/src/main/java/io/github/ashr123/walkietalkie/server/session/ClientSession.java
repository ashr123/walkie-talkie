...
public interface ClientSession {
	...
	/// The endpoint this session dialled — `/ws/audio` or `/ws/signal`. It is an opening bid, not a verdict: it
	/// seeds a channel this session CREATES, and after that the CHANNEL's transport
	/// ([Channel#transport()]) is the one that decides how this
	/// member's audio travels. Read the channel, not this, anywhere a member's media plane matters — a joiner
	/// adopts its channel's plane, and an owner can move a channel under it.
	Transport transport();
	...
	/// already-closed session before it resurrects per-session state (e.g. a [io.github.ashr123.walkietalkie.server.ratelimit.SessionRateLimiter] bucket after
