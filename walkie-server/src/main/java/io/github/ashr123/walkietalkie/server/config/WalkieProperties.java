...
/// @param floorIdleReleaseSeconds push-to-talk idle auto-release: reclaim the floor for a new requester when
///                               the current holder has sent no audio for this many seconds (0 disables).
///                               Measured from frame timing only, so it works on encrypted channels
/// @param floorMaxHoldSeconds    push-to-talk max-hold cap: force-release the floor after this many seconds of
///                               continuous holding, so no one can hog it indefinitely (0 disables)
