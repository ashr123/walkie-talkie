...
class WebSocketRelayIntegrationTest extends WebSocketIntegrationTestSupport {
	...
	void pushToTalkFloorGrantAndAudioFanOut() throws Exception {
		...
		try (WebSocketSession sessionA = connect(AUDIO, handlerA, tokenA);
			...
			send(sessionA, new ClientMessage.Join("team", ChannelMode.MULTI_CHANNEL_PTT, null, "Alice", TestKeyChecks.ENCRYPTED));
			ServerMessage.Joined joinedA = awaitType(handlerA.messages, ServerMessage.Joined.class);
			assertEquals("team", joinedA.channel());

			send(sessionB, new ClientMessage.Join("team", ChannelMode.MULTI_CHANNEL_PTT, null, "Bob", TestKeyChecks.ENCRYPTED));
