...
4. implement end-to-end encryption (§7), byte-compatibly with the reference clients — mandatory for every
   channel except the server-managed `global` room, on both transports.
...
| `type`                   | Fields                                                    | Meaning                                                                                                                                                                                                                                                                                                                                                                                               |
|--------------------------|-----------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `join`                   | `channel`, `mode`, `transport`, `displayName`, `keyCheck` | Join/create — or **switch** channel in place when re-sent on a live socket (§3c). `keyCheck` is REQUIRED for every channel but `global`, which refuses it (§7). `transport` (`AUDIO_RELAY` \| `SIGNALING`) is the media plane you want, honoured only if this join CREATES the channel — otherwise you adopt the channel's, which `joined` reports; send `null` to mean "the endpoint I dialled" (§8) |
...
| `changeTransport`        | `transport`                                               | Owner-only: move the WHOLE channel to the other media plane, `AUDIO_RELAY` ⇄ `SIGNALING` (§8). Nobody reconnects and nobody's session id, floor or roster position changes — the two endpoints speak the same control protocol. Naming the plane the channel already uses is accepted and does nothing (→ `transportChanged`)                                                                         |
| `rename`                 | `displayName`                                             | Change your own display name in place (→ `memberRenamed`, §3c)                                                                                                                                                                                                                                                                                                                                        |
| `changePassphrase`       | `keyCheck`, `wrappedKey`                                  | Owner-only: rotate the channel passphrase; `keyCheck` = the new one's KCV, and must be non-null — clearing it is refused with `PASSPHRASE_REQUIRED` (§7). Optional `wrappedKey` = the new passphrase encrypted under the OLD key so members auto-adopt; `null` opts out (§3c) (→ `passphraseChanged`)                                                                                                 |
...
| `type`                  | Fields                                                                                                            | Meaning                                                                                                                                                                                                                                                                                                                                                                                                               |
|-------------------------|-------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `joined`                | `selfId`, `channel`, `mode`, `transport`, `ownerId`, `locked`, `floorQueueEnabled`, `muteNewMembers`, `members[]` | Join ack + full snapshot (re-sync on every join); `transport` = the channel's media plane, which you ADOPT — build the pipeline it names, not the one your endpoint implies (§8); `locked` = channel locked to new members (§3e); `floorQueueEnabled` = the PTT floor queue is on (§3b); `muteNewMembers` = arrivals are muted (§3d). YOUR own mute is in your `members[]` entry. A `floorStatus` follows immediately |
...
| `transportChanged`      | `transport`                                                                                                       | The owner moved this channel to the other media plane (§8). Rebuild your LOCAL audio for it and do NOT reconnect — your session, roster position, stream index and the floor all survive. A client that cannot speak the new plane should say so and `leave`                                                                                                                                                          |
...
| `passphraseChanged`     | `keyCheck`, `wrappedKey`                                                                                          | The owner ROTATED the channel passphrase; `keyCheck` is always non-null (§3c). If `wrappedKey` is present, decrypt it with your old key to auto-adopt; else re-derive from the out-of-band passphrase and verify against `keyCheck`                                                                                                                                                                                   |
...
**A channel is single-transport, and the CHANNEL decides — not your socket.** The endpoint you dial is an
opening bid: it seeds a channel you create (as does `join.transport`, which wins when you send it), but joining
an existing channel means adopting its plane, whatever you dialled. `joined.transport` is the answer; build the
pipeline it names. There is no mismatch to be refused, because you get no vote.

The two media planes never meet, so a mixed channel would carry no audio in either direction while looking
entirely healthy (full roster, working floor control) — which is why it is made impossible rather than warned
about. A signal sent in a channel that is not on the WebRTC plane is dropped silently for the same reason, and
because answering an offer attaches a microphone to media that takes neither the server's floor and owner-mute
enforcement nor the passphrase E2EE, both of which exist only on the relay frame path.

The channel's owner can move it between planes at any time with `changeTransport`; everyone gets
`transportChanged` and rebuilds their audio in place. **Do not reconnect on it.** Both endpoints speak the
identical control protocol — `/ws/audio` is `/ws/signal` plus a binary handler — so your session id, your place
in the roster, your stream index and the floor all survive the move, and a reconnect would throw away every one
of them (including, for the owner, ownership itself). A client that cannot speak the new plane should say so and
send `leave`.
...
`changePassphrase`, whose `keyCheck` is the KCV (§7) of the **new** passphrase and MUST be non-null: a null one
is refused with `PASSPHRASE_REQUIRED` and the channel keeps the passphrase it had, because no channel may be
plaintext (§7). The server records the new key-check and broadcasts `passphraseChanged { keyCheck, wrappedKey }`
to **all** members (including the owner). The passphrase itself is **never** sent to the server in clear. On
`passphraseChanged`:

- `keyCheck` is never `null`. If you receive one that is, treat it as a protocol violation and **keep the key you
  hold** — do NOT drop it. Dropping it would put you back to sending in the clear, so obeying such a message is a
  downgrade, and both reference clients refuse it. (This is a change: it used to mean "the owner turned encryption
  off", which is no longer an operation.)
- Otherwise adopt the new passphrase — two ways:
  ...
  - **Manual (when `wrappedKey` is absent or undecryptable).** Re-derive your AES key from the new passphrase
    ...
    **revocation-style** rotation (see the caveat below). Every rotation has an old key to wrap under, since a
    channel is created encrypted and cannot be cleared; only the channel's FIRST passphrase travels out-of-band.
- **Until you hold a key whose KCV equals the announced `keyCheck`, you are muted:** suppress transmission (send
  neither plaintext nor stale-key ciphertext) and you can't decode others. The live case is a **stale-key
  straggler** after a rotation you haven't adopted (your old key no longer matches); the rule also covers holding
  **no** key for a channel that announces one, which a conformant server no longer produces and is therefore the
  shape a downgrade attempt takes. A conformant client MUST gate its send path on "the KCV of the key I hold
  equals the channel's announced `keyCheck`" — not merely "I hold *some* key" — so a straggler can't emit audio
  the re-keyed channel can't decode.
- **Decide "may I send plaintext?" from YOUR OWN state, never from the announced `keyCheck`.** Send in the clear
  only when *both* (a) you are in the `global` room and (b) you never derived a key. Inferring it from
  `keyCheck == null` looks equivalent and is not: that value comes from the server, so a single forged
  `passphraseChanged { keyCheck: null }` would flip every member of an encrypted channel into transmitting
  cleartext. The second condition matters too, because the channel mode also arrives from the server — holding a
  key you derived yourself means encryption was intended, and must win over anything the server claims. The
  reference clients implement this as a pure decision (`frameDisposition` / `outboundFrame`) taking the
  "plaintext allowed" flag as an argument, precisely so it cannot be re-derived from server input by accident.
- Refuse to send audio at all when you are not in a channel: the channel-less state is where clients typically
  clear their record of the announced key-check, so a send path gated only on "the socket is open" can leak.
...
## 7. End-to-end encryption (mandatory outside the `global` room)

**Every channel is end-to-end encrypted.** A `join` naming any channel other than `global` MUST carry a
`keyCheck`, and one that does not is refused with `PASSPHRASE_REQUIRED` — there is no way to create or enter a
plaintext channel, and no way for an owner to turn encryption off afterwards. The server-managed `global` room is
the sole exception and is the exact inverse: it refuses a `keyCheck` with `ENCRYPTION_NOT_ALLOWED`.

This applies on **both transports**. WebRTC media is already peer-to-peer encrypted (DTLS-SRTP) and never reaches
the server, so a WebRTC client's key never encrypts a frame — but it must still derive one and send its
`keyCheck`, because that is what the server matches a joiner against. Two consequences worth knowing: the
passphrase acts as a channel membership credential on the WebRTC path, and a relay member and a WebRTC member can
share a channel (their key-checks agree, where a null one could never match an encrypted channel).

On the relay path, the sender encrypts the **whole** `[codec tag][payload]` plaintext and the body becomes
`[0xE2][IV(12)][AES-256-GCM ciphertext+tag]`. Must be **byte-identical** across clients:
...
- **Key-check:** send the hex KCV in `Join.keyCheck` — required for every channel but `global`. The server
  enforces a **uniform** channel (all members the same passphrase) and rejects a mismatch with
  `error: PASSPHRASE_MISMATCH`, comparing the KCV without ever learning the passphrase. A missing one is a
  different error, `PASSPHRASE_REQUIRED`: mismatch means you disagree with a channel's key, missing means you
  brought none at all — and the second is what used to quietly create a plaintext channel.
- **Rotation:** the channel **owner** may change the passphrase mid-session with `changePassphrase` (§3c),
  whose `keyCheck` is the KCV of the **new** passphrase. It can only ever be rotated to another key: a `null`
  `keyCheck` is refused with `PASSPHRASE_REQUIRED` and the channel keeps the passphrase it had. The server swaps
  the recorded KCV and broadcasts `passphraseChanged`; it still never sees the passphrase. Members adopt
...
| `PASSPHRASE_REQUIRED`      | A `join` to any channel other than `global` carrying `keyCheck: null`, or an owner's `changePassphrase` with `keyCheck: null` (there is no way to turn encryption off). Every ordinary channel is end-to-end encrypted, so derive a key from a passphrase and send its key-check (§7). Not transient — retrying without one fails identically. The inverse of `ENCRYPTION_NOT_ALLOWED`                                                                                                                                                                       |
